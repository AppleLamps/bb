...
// The config layer joins and resolves data-dir paths with node:path against
// the running platform, so a POSIX fixture like "/Users/tester" comes back
// separator-normalized (and drive-qualified) on Windows. Fixture *inputs* stay
// POSIX literals on purpose: the dev instance id hashes the raw repoRoot
// string, so resolving them would move the pinned hashes below.
describe("common config", () => {
  ...
  it("resolves development defaults from the checkout instance", () => {
    ...
    ).toBe(path.join("/Users/tester", ".bb-dev", "src-bb-9039de53a76a"));
  });
